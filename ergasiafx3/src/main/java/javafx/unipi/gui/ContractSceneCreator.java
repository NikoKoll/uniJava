...
public class ContractSceneCreator extends SceneCreator implements EventHandler<MouseEvent> {
    ...
    public ContractSceneCreator(double width, double height) {
        ...
        TableColumn<Contract, String> finalCostColumn = new TableColumn<>("Final Cost");
        finalCostColumn.setCellValueFactory(new PropertyValueFactory<>("finalCost"));
        contractTableView.getColumns().add(finalCostColumn);
        
        TableColumn<Contract, String> discountColumn = new TableColumn<>("Discount");
        discountColumn.setCellValueFactory(new PropertyValueFactory<>("discount"));
        contractTableView.getColumns().add(discountColumn);
        
        TableColumn<Contract, String> cancellationCostColumn = new TableColumn<>("Cancel Cost");
        cancellationCostColumn.setCellValueFactory(new PropertyValueFactory<>("cancellationCost"));
        contractTableView.getColumns().add(cancellationCostColumn);
		...
		if (event.getSource() == newContractBtn) {
		    ...
		    try {
				...
				//	System.out.println("1");
					if (contractList.get(i).getAfm() != afm) {
				//		System.out.println("2");
						if (phoneString.length() == 10 && afmString.length() == 9) {
				//			System.out.println("3");
							if (contractList.get(i).isActiveContract()==true && contractList.get(i).getPhoneNumber()!=phoneNumber) {
			...
			if (activeContract == true) {
                ...
                for (Contract c : contractList) {
                	...
                	if (c.getAfm() == Integer.parseInt(afmField.getText())) {
        		    	...
        		    	long enddate = startDate - month;
        		    	
        		    	if(enddate <= 3) {
        		    		c.setCancellationCost(enddate);
        		    	}
...
public void createContract (String code, long phoneNumber, long afm, String programType, long startDate, int contractDuration, double discount, double finalCost, String accountType, String paymentMethod, double cancellationCost, boolean activeContract) {

	List<Client> listclient = ClientSceneCreator.getClientList();
	List<Plan> listplan = PlanSceneCreator.getPlanList();
	int i;
	double calls[] = {0,0};
	double finalc[] = {0,0};
	
	for(i=0; i<listplan.size();i++) {
		if(programType.equals("Mobile")) {
			calls[0] = 0;
			finalc[0]= listplan.get(i).getProgramCost();
		}
		if(programType.equals("Landline")) {
			calls[1] = 0;
			finalc[1]= listplan.get(i).getProgramCost();
		}
		if((listplan.get(i).getCallMinutes() > 1000) && programType.equals("Mobile")) {
			calls[0] = 11;
		}
		if((listplan.get(i).getCallMinutes() > 1000) && programType.equals("Landline")) {
			calls[1] = 8;
		}
	}
	...
	for(i=0; i<listclient.size();i++) {
		...
		if(listclient.get(i).getAfm() == afm) {
			
			if(listclient.get(i).getJob().equals("Pro")) {	
				
				discount = discount+10;
				
			}else if(listclient.get(i).getJob().equals("Student")) {
			...
			}else if(paymentMethod.equals("Electronic Account")) {
			...
			if(programType.equals("Mobile")){
				...
				discount = calls[0]+discount;
				finalCost = finalc[0] - finalc[0]*discount/100;
				
			}else if(programType.equals("Landline")) {
				
				discount = calls[1]+discount;
				finalCost = finalc[1] - finalc[1]*discount/100;
