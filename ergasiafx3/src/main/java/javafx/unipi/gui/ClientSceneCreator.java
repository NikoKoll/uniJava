...
public class ClientSceneCreator extends SceneCreator implements EventHandler<MouseEvent> {
	...
	static ArrayList<Client> clientList; //arxikopoihsh listas typou client
	...
	public void handle(MouseEvent event) {
		...
		if (event.getSource() == backBtn) { //back button
			...
			App.primaryStage.setTitle("Telecommunication System "
					+ "Windows");
		...
		if (event.getSource() == deleteClientBtn) { //diagrafh pelath
			...
			for(i=0;i < list.size(); i++) {
				if(afm == list.get(i).getAfm()) { //elenxos gia an yparxei plan me thn eteairia pou diagrafetai
					delete = false;
				}
			}
			
			if(delete==true)
			deleteClient(licensenumberField.getText());
    ...
    public void deleteClient(String licensenumber) {
        ...
        for (int i = 0; i < clientList.size(); i++) {
            ...
            if (clientList.get(i).getLicensenumber().equals(licensenumber)) {
