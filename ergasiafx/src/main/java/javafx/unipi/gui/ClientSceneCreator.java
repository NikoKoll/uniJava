...
public class ClientSceneCreator extends SceneCreator implements EventHandler<MouseEvent> {
	...
	//buttons
	Button newClientBtn, searchClientsBtn, modifyClientBtn, deleteClientBtn, backBtn, searchBtn, searchBtn2;
	...
	public ClientSceneCreator(double width, double height) {
		...
		licensenumberLbl = new Label("Αριθμός Ταυτότητας: ");
		afmLbl = new Label("ΑΦΜ: ");
		nameLbl = new Label("Όνομα: ");
		surnameLbl = new Label("Επίθετο: ");
		jobLbl = new Label("Επάγγελμα: ");
		adressLbl = new Label("Διεύθυνση: ");
		...
		newClientBtn = new Button("Δημιουργία");
		searchClientsBtn = new Button("Αναζήτηση");
		modifyClientBtn = new Button("Τροποποίηση");
		deleteClientBtn = new Button("Διαγραφή");
		...
		//customize TableView
		TableColumn<Client, String> licensenumberColumn = new TableColumn<>("Αριθμός Ταυτότητας");
		...
		TableColumn<Client, String> afmColumn = new TableColumn<>("ΑΦΜ");
		...
		TableColumn<Client, String> surnameColumn = new TableColumn<>("Επίθετο");
		surnameColumn.setCellValueFactory(new PropertyValueFactory<>("surname"));
		clientTableView.getColumns().add(surnameColumn);
		
		TableColumn<Client, String> jobColumn = new TableColumn<>("Επάγγελμα");
		jobColumn.setCellValueFactory(new PropertyValueFactory<>("job"));
		clientTableView.getColumns().add(jobColumn);
		
		TableColumn<Client, String> adressColumn = new TableColumn<>("Διεύθυνση");
	...
	public void handle(MouseEvent event) {
		...
		if (event.getSource() == backBtn) { //back button
			...
			App.primaryStage.setTitle("Καταστημα Παροχης Τηλεπικοινωνιακων Υπηρεσιων");
			App.primaryStage.setScene(App.mainScene);
		}
		
		if (event.getSource() == newClientBtn) { //dhmiourgia neou pelath
			
			licensenumber = licensenumberField.getText();
            ...
            try { //elegxos an to afm kai to phone einai Strings
            	...
            	int phone = Integer.parseInt(phoneField.getText());
    				...
    				for(i=0; i < clientList.size(); i++ ) {
    					if((clientList.get(i).getLicensenumber().equals(licensenumber) && clientList.get(i).getAfm() != afm) || clientList.size() == 0) {
            ...
            //???????? phonelength==10 kai monadiko afm kai adt
	...
	public void createClient(String licensenumber, int afm, String name, String surname, String job, String adress, String email, int phone) {
    ...
    public void updateClient(String licensenumber, String name, String surname, String job, String adress, String email, int phone) {
